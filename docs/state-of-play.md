...
| Simon | **Story mode**: a brand-new linear narrative feature, not a curriculum variant — role-play a tarot reading in first person as a client (recurring across chapters) visits. A chapter has a location, a character, and a fixed sequence of dialogue and 2-4-option multiple-choice beats plus scripted card reveals; no branching. Reachable from the dev console as its own top-level group beside Path (both now collapse to an accordion, `0049`), listing every chapter by name (`0052`). Real art now drives the scene (`public/assets/reading-scene-sketch-v2/`): a tilted table, a character per side with emotion states, a client's line in a speech bubble anchored to their face, and an animated card reveal (lift, flip, grow tall on whichever side is clear; shrinks to rest beside the pile on dismissal; fades out when the chapter ends). Two chapters exist — Dave's (`chapter-01`, fully scripted, grounded in real card talking points) and a deliberately minimal Riley stub (`chapter-02`) Simon is writing himself. The dev console's skip controls work here too, and the progress bar counts decisions (choice/draw beats) rather than every beat. See [`0048`](decisions/0048-story-mode-a-linear-narrative-reading.md), [`0049`](decisions/0049-dev-console-groups-collapse.md), [`0050`](decisions/0050-story-gets-real-art-and-a-second-chapter.md), [`0052`](decisions/0052-story-dev-tools-and-a-decision-based-progress-bar.md) | **In progress** — 31 Aug, branch `story-mode`, two chapters, not yet merged |
