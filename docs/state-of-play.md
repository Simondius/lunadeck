...
| Claude | **v4**: a path merging Story mode's narrative with v3's lesson content, **now the default at `/`** — eight units. Six pair one character (Dave or Riley) with the card their narrative just introduced (Fool, Lovers, Empress), shaped as start-narrative → five lesson nodes → end-narrative (down from seven, the two most repetitive nodes per card cut). Two more (7, 8) are cumulative-review units whose three lesson steps span all three cards, wrapped in a Dave-at-the-start/Riley-at-the-end crossover narrative — a real three-card reading each, not just a framing stub. v3's Fool/Lovers/Empress lesson content itself is untouched (`data/v4/*_section.json`, regrouped from 13-14 nodes each down to 7 by `scripts/build-v4-sections.mjs`, no rounds dropped); a review unit's lesson step just links straight at one card's own existing node/route rather than a new merged file. New dramatic "character node" icon for narrative steps; the trail's first companion image per unit is the card itself, later ones the character. v1, v2, and v3 all still live on behind the dev console. See [`0057`](decisions/0057-v4-merges-story-mode-and-v3-lessons.md), [`0058`](decisions/0058-v4-review-units-and-crossover-narrative.md) | **In progress** — 2 Sep, branch `v4-narrative-lessons`, not yet merged |
