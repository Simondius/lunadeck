...
| who | what | status |
| --- | --- | --- |
| Claude | New top-level mode, **Journey** - a narrative-driven alternative to Path, now the default at `/` (Path/v4 unchanged, reachable via the dev console's "Open Path (legacy)" link). Ships with a global CSS animation library (`app/globals.css`, "Journey mode") driving text-reveal/card-breathe/choice-resolve behaviour from one place, and unit 1 - The Fool - built from Tia's 27-screen mock walkthrough, art extracted into `assets/journey/fool/`. Separate progress store (`lunadeck.journey.v1`, `lib/journey-progress.js`); narrative-progress/deck-progress gating is not wired up yet. See [`0100`](decisions/0100-journey-mode-scaffold.md) | **Merged** 6 Sep - #98 |
| Claude | Tia wanted the UI as PNGs for hand-layout in Figma. `scripts/export_ui.mjs` (`npm run export:ui`) drives Playwright over the dev server and writes 12 screens and 18 transparent cutouts into gitignored `exports/figma/`, at 3x, from a manifest of route+selector - plus `.svg` for the pieces that are inline vector already. The card art needed nothing: `assets/` was already 358 PNGs. Three pieces we had listed turned out to be dead CSS (`.badge-right`/`.badge-wrong` in the orphaned `components/lesson/options.jsx`, and `.board-column`, which has no consumer at all). See [`0098`](decisions/0098-the-ui-exports-itself-for-figma.md) | **Merged** 4 Sep - #89 |
| Claude | Three leftovers from `0082`: the last file in `components/lesson/`, the dead `.board`/`.tile` and `.grid`/`.option`/`.badge` CSS, and a comment in `app/page.js` still promising v1-v3 routes. **One of the three was not dead** — `options.jsx`'s `Inspector` is live on every cloze round, hidden from the grep that reported it orphaned by that grep's own `-v lesson-v2` filter. `Inspector` moved to `components/lesson-v2/inspector.jsx`; everything else went, 191 lines of CSS included. **`specs/Design_Handoff_Violet.md` is fixed in the same PR** — annotated, not rewritten: its design tokens are still authoritative and were checked against `:root` line by line, while everything from *Screens* onward describes the v1 build and now says so, section by section, with a table mapping it onto the app today. Checking it found that **two of the four additions the redesign was commissioned for were never built** — progress rings on the path, and the overall curriculum progress bar. Its own decision doc collided on number with the Figma-export row above (both claimed `0098` independently); renumbered to [`0099`](decisions/0099-the-last-crumbs-of-v1-v3-and-the-one-that-was-not-dead.md) | **Merged** 5 Sep - #91 |
| Claude | Removing v1-v3 (`0082`) took both data checks with it, leaving 501 v4 rounds and 551 story beats unchecked. `scripts/check_v4_rounds.mjs` replaces them, wired into `npm test`, and it found five rounds sharing an id inside a node. See [`0097`](decisions/0097-v4s-rounds-get-a-checker-again.md) | **Merged** 4 Sep - #88 |
...
| Claude | **v1, v2, v3, and the standalone Story mode are removed** — v4 has been the app's only path since 3 Sep, so `app/v1`, `app/v2`, `app/v3`, `app/units`, `app/story/page.js`, their own data (`data/v2`, `data/v3`, `data/data_curriculum_nodes.csv`, `data/data_unit_metadata.csv`, `data/story/chapter-0{1-4}.json`), and the code that only ever served them (`components/lesson/*` — all of it, once `0098` finished the job, `components/path-screen.jsx`, `scripts/check_data.py`, `scripts/check_rounds.mjs`, `scripts/build-v4-sections.mjs`) are gone. `/story/play/[chapter]` survives as v4's own narrative-node player route — only the flat `/story` index and its four original chapters went. `lib/data.js`'s `getPath`/`getAllSections` (Deck, Reader, Social, dev console) are rewritten against v4's own `data/v4/units.js`/`sections.js` instead of the deleted CSVs — same shape out, so nothing downstream changed. Two bugs found along the way, both fixed: the course's last unit (World) skipped its own unlock celebration because that screen required a "next step" that doesn't exist after the last unit; and the dev console's default-minimized position could land tens of thousands of pixels down the page on the long single-scroll path, since it measured `.app-frame`'s own (unbounded, on mobile) content height instead of the viewport. See `0082`. **Follow-up fixes on `v4-followups`**: distractor variety across the whole course — no silly word repeats more than 2x per card/5x total, no single donor card backs more than ~10 of the real-phrase "wrong answer" slots (Fool alone used to back 35), every cloze round carries at least 2 distractors, a node-2 regeneration regression fixed at the source (`0088`, `0091`, `0092`). The mid-unit capstone is renamed "Alignment Check" (`0089`) and now opens and closes on its own ceremony — the card grows from 20% size wobbling and shimmering while three stars swirl into the same line its own path icon shows, a card-specific pun, then a mirrored resolution screen — live on all 22 cards (`0093`). | **In progress**, branch `v4-followups` |
