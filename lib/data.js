...
import { buildNode, coreKeywords } from "./rounds.js";
  ...
  units.forEach((unit, i) => {
    ...
    for (const section of perUnit[i]) {
      path.push({
        ...
        // A recap teaches no single card, so it wears the unit's own icon.
        image: section.cardKey ? circleForKey(section.cardKey) : unit.icon,
  ...
  return {
    ...
    symbol: symbol
      ...
      : card.suit
        ? `Suit: ${card.suit}`
        : null,
    // The core set only — the same one the section's first exercise asks for.
    // The full list is on the card's page in the deck.
    keywords: coreKeywords(keywords.get(cardKey) ?? []),
...
// The same idea for exercise options, with the requirement inverted: pick a
// sentence that does NOT name the card. UX Style Guide Section 4 — a candidate
// must never carry the identity that answers the question, even when the same
// name is shown on the fixed reference above it. description_anonymized exists
// for exactly this, and the sentence is selected from it, never rewritten.
function anonymousOpener(cardName, text) {
  ...
  const names = (sentence) =>
    own.some((w) => new RegExp(`\\b${w}\\b`, "i").test(sentence));

  return sentences.find((sentence) => !names(sentence)) ?? sentences[0];
}
...
export async function getSession(unitNumber, section = null) {
  ...
  // The opener needs the card's name to pick the right sentence, so it is
  // attached here where both are in hand.
  for (const [key, entry] of descriptions) {
    const name = cards.names.get(key) ?? "";
    // Named, for teaching: "The Fool stands at both the start and end…"
    entry.opener = opener(name, entry.condensed);
    // Unnamed, for exercise options: "This character stands at both…"
    entry.anonOpener = anonymousOpener(name, entry.anonymized);
  }
