...
// The reader deals three, not one. Each pick reuses the weighted table above
// with a per-slot seed, and excludes what the earlier slots already took — so
// the three are distinct, the pool weights still hold, and the whole spread is
// still fixed by the calendar date.
//
// Returns fewer than `count` only when the deck genuinely runs out.
export function pickDailyDraw({
  cards,
  knownKeys = [],
  drawnKeys = [],
  reversedKeys = [],
  ...
  for (let i = 0; i < count; i++) {
    const pick = pickDraw({
      cards: cards.filter((c) => !taken.has(c.key)),
      knownKeys,
      drawnKeys,
      reversedKeys,
      currentUnit,
