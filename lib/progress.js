...
function readDailyDraw(value) {
  if (value.dailyDraw && Array.isArray(value.dailyDraw.cards)) return value.dailyDraw;
...
export function recordDailyDraw({ cards = [], day = today() } = {}) {
  ...
  const next = {
    ...
    dailyDraw: {
      date: day,
      cards: dealt.map((c) => ({ cardKey: c.cardKey, reversed: !!c.reversed })),
    },
    drawnCardKeys: [...drawn],
...
export function recordReading({ question, cards = [], reading, day = today() } = {}) {
  const state = read();
  if (!reading) return state;
  const next = { ...state, lastReading: { date: day, question, cards, reading } };
  write(next);
  return next;
}
