  ...
  return [
    {
      ...
      reference: { type: "label", text: `${target.type}: ${target.name}` },
...
function formatB(node, ctx) {
  const statementFor = (key, seed) =>
    pickSeeded(ctx.talkingPoints.get(key) ?? [], seed);
  ...
  return node.cardsInvolved
    .map((shown, index) => {
      ...
      const statement = isTrue
        ? statementFor(shown, seed)
        : statementFor(donorKey, `${seed}-donorpoint`);
...
export function buildNode(node, ctx) {
  ...
  return instances && instances.length > 0 ? instances : null;
}
