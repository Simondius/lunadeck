...
// How many of a card's keywords the course actually holds a learner to on a
// first meeting. Cards carry two to seven; most carry three or four, and being
// asked for seven at once on the first screen of a card is not a first meeting,
// it is a vocabulary test. The rest stay on the card's own page in the deck —
// 0012's rule ("depth arrives after the gist"), applied to keywords. The intro
// screen shows the same core set, so the course teaches exactly what it tests.
export const CORE_KEYWORD_COUNT = 3;

// Keywords arrive in the CSV's own keyword_order, so the first are the ones
// the guidebook leads with.
export function coreKeywords(list) {
  return list.slice(0, CORE_KEYWORD_COUNT);
}

// Two lines of chips, on the narrowest phone the app targets. A fixed chip
// count can't deliver that — "levity" and "openness of the heart" do not cost
// the same — so the round packs its own set and stops at two rows.
//
// 350px is the .chips container at a 390px viewport, measured in the built app.
// The width numbers are not a fit but an upper bound: every one of the 206
// distinct chips the deck can produce was measured in a browser at both sizes,
// and the constants are the tightest line that sits above all of them. A fit
// is not good enough here — a proportional font puts "community" 4px above its
// own trend line, which is the difference between two rows and three.
// check_rounds.mjs asserts the outcome for all 78 rounds, so if the chip
// styling changes and these drift, the build says so.
const CHIP_ROW_WIDTH = 350;
const CHIP_GAP = 9;
const CHIP_ROWS = 2;

// A handful of minor-arcana cards keep whole clauses as keywords — "inner
// obstruction we cause ourselves" — and no chip count short of two makes those
// fit at the normal size. Those rounds render a size down instead of spilling
// onto a third line.
function chipWidth(text, compact) {
  return compact ? 6.1 * text.length + 32 : 6.1 * text.length + 47;
}

// Mirrors flex-wrap: fill a row left to right, break when the next chip
// doesn't fit. justify-content only moves chips within a row, so it can't
// change where the breaks land.
export function fitsTwoRows(texts, compact = false) {
  let rows = 1;
  let used = 0;
  for (const text of texts) {
    const width = chipWidth(text, compact);
    const needed = used === 0 ? width : used + CHIP_GAP + width;
    if (needed <= CHIP_ROW_WIDTH) {
      used = needed;
    } else {
      rows += 1;
      used = width;
      if (rows > CHIP_ROWS) return false;
    }
  }
  return true;
}

// What a round assumes the learner has already been shown, and about which
// cards. The play page reads this to decide where a teaching beat belongs; it
// means nothing on its own, and a round that needs nothing taught (Major or
// Minor Arcana, say) simply doesn't carry it.
function needs(topic, cards) {
  return { teaches: topic, teachesFor: cards };
}
...
// The set starts at one more wrong than right, so the answer is never simply
// "take half", and comes off from there until it fits two lines. Wrong chips
// go first — a round with two distractors is still a question, a round with
// one keyword to find is barely one — and the normal chip size is exhausted
// before the smaller one is tried.
function packChips(node, core, ordered) {
  const floor = Math.min(2, core.length);
  for (const compact of [false, true]) {
    for (let n = core.length; n >= floor; n--) {
      const shown = core.slice(0, n);
      for (let w = shown.length + 1; w >= 2; w--) {
        const wrong = ordered.slice(0, w);
        if (wrong.length < w) continue;
        const chips = mixChips(node, shown, wrong);
        if (fitsTwoRows(chips.map((c) => c.text), compact))
          return { chips, shown, compact };
      }
    }
  }
  // Nothing fits even compact: take the smallest honest set and let it wrap.
  const shown = core.slice(0, floor);
  return { chips: mixChips(node, shown, ordered.slice(0, 2)), shown, compact: true };
}

function mixChips(node, correct, wrong) {
...
function formatKeywords(node, ctx) {
  const all = ctx.keywords.get(node.cardKey) ?? [];
  if (all.length === 0) return null;
  let correct = coreKeywords(all);

  // Distractors are checked against the card's *whole* keyword set, not just
  // the core: a keyword this round chose not to ask for is still a right
  // answer, and offering it as a wrong one would be a lie.
  const own = new Set(all.map((k) => k.toLowerCase()));
  ...
  const ordered = [...fits, ...rest];
  const packed = packChips(node, correct, ordered);
  const chips = packed.chips;
  correct = packed.shown;

  return [
    {
      kind: "K",
      ...needs("keywords", [node.cardKey]),
      ...
      chips,
      // Set when the card's keywords are long enough that two lines needs a
      // size down. Only a few minor-arcana cards trip it.
  ...
  return [
    {
      ...
      answerKey: node.cardKey,
      ...needs(useTalkingPoints ? "notes" : "meaning", [node.cardKey]),
      revealTitle: node.cardName,
      revealBody: ctx.descriptions.get(node.cardKey)?.condensed ?? "",
    },
  ];
