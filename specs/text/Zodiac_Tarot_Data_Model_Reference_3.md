# Zodiac_Tarot_Data_Model_Reference_3
*Converted from `Zodiac_Tarot_Data_Model_Reference_3.pdf`.*
...
Page 1
Zodiac Tarot App — Card Data Model Reference
Zodiac Tarot App
...
deck's identity, symbols, keywords, guidebook text, reversed-card interpretation, birth card lookup, curriculum
structure, and account/social systems. It's written for a technical reader comfortable with spreadsheets and
databases, but who doesn't need formal schema terminology.
This document covers two layers, built in two different threads. The tarot content layer (Sections 2-12) is the
deck itself. The app/curriculum layer (Sections 13-15) is how that content becomes lessons, units, and accounts -
built afterward, on top of the content layer, reading from it by column name rather than duplicating it.
Companion files: zodiac_tarot.db (all tables, images embedded as BLOBs) and "Script to extract the images from DB.py" (pulls images
back out to files). See Section 16.
...
Zodiac Tarot App — Card Data Model Reference
1. The big picture
The data is organized as flat tables (think: spreadsheet tabs), each focused on one kind of information. Every card
has a unique ID, card_key (e.g. major_00_fool), which ties tables together. Curriculum tables add two more keys:
unit_number and node_id.
Tarot content layer
Table
Rows
What it is
2. Card Base List
78
Card identity + image filename.
3. Major Arcana Symbols
22
Planet/sign glyph per Major card.
4. Minor Arcana Attributes
56
Glyphs/banner text per Minor card.
5. Card Keywords
307
Keywords, one per row.
6. Card Talking Points
345
Reading notes, one per row.
7. Symbol Significance
137
Planet/sign/element/suit glossary.
8. Card Descriptions
78
Verbatim, condensed, reversed, and anonymized text per card.
9. Symbol Images
30
Symbol name -> icon filename.
10. Birth Card Lookup
21
Birth-number -> Major Arcana card.
11. Card Similarity
6,006
Pairwise similarity for lesson difficulty.
12. Teaching Order
78
Curriculum sequence, easiest to hardest.
App / curriculum layer (from the design thread)
Table
Rows
What it is
13. Unit Metadata
10
One row per curriculum unit.
14. Curriculum Nodes
577
One row per playable lesson node.
15. Account & Social (Meta
Systems)
0 + 78
users (schema only) + avatar_sources (78 rows).
Two more tables exist only inside the database, not as their own CSV: Card Images (every card's artwork
embedded as a BLOB) and the image data embedded directly inside avatar_sources and symbol_images. This
makes the database a single, fully self-contained file.
An important design boundary, held throughout
Tables 2-4 contain only what's physically printed on a card face. Interpreted meaning lives separately, in Tables 5-8.
Runtime data (accounts, progress) is never mixed with authored content - Section 15's users table ships with zero
rows, since it's populated by the app, not by this data-building process.
...
Zodiac Tarot App — Card Data Model Reference
2. Card Base List
File: data_tarot_cards_base.csv | 78 rows | 1 row per card
Column
Meaning
card_key
Unique ID, e.g. major_04_emperor or minor_wands_02.
arcana_type
"major" or "minor".
suit
Wands/Cups/Swords/Pentacles for Minor Arcana; blank for Major Arcana.
rank
0-21 for Major Arcana; Ace/2-10/Page/Knight/Queen/King for Minor Arcana.
card_name
Full printed name.
image_file
Card artwork filename, e.g. major_00_fool_MASTER.png.
Full data: Appendix A.
3. Major Arcana Symbols
File: data_major_arcana_symbols.csv | 22 rows | 1 row per Major card
Every Major Arcana card shows exactly one astrological glyph: a planet or a zodiac sign, confirmed both from the
guidebook and by visual inspection of the artwork.
Column
Meaning
symbol_type
"Planet" or "Zodiac Sign".
symbol
The specific name, e.g. "Mercury" or "Aries". Matches symbol_name in Symbol
Significance/Images.
10 Planet cards, 12 Zodiac Sign cards. Full data: Appendix B.
4. Minor Arcana Attributes
File: data_minor_arcana_attributes.csv | 56 rows | 1 row per Minor card

Aces (4): three zodiac glyphs - the signs sharing the suit's element.

...
Glyph pattern confirmed by direct inspection of 5 of 56 images, applied to the rest via the guidebook's stated planet/sign
list. Full data: Appendix C.
5. Card Keywords
File: data_card_keywords.csv | 307 rows | multiple rows per card
One row per keyword. source is "guidebook" or "proposed" (2 cards - Queen of Cups, Ace of Swords - have no
guidebook keyword line and were authored to fill the gap). Full data: Appendix D.
6. Card Talking Points
File: data_card_talking_points.csv | 345 rows | multiple rows per card
Longer-form reading notes, one per row. Full 78-card coverage, no gaps. Full data: Appendix E.
...
File: data_symbol_significance.csv | 137 rows | multiple rows per symbol
A standalone glossary of what each Planet/Sign/Element/Modality/Suit means on its own, independent of any one
card. Full data: Appendix F.
...
Zodiac Tarot App — Card Data Model Reference
8. Card Descriptions
File: data_card_descriptions.csv | 78 rows | 1 row per card
Column
Meaning
astrological_label
Heading under the card title, e.g. "URANUS" or "MARS IN ARIES, FIRST DECAN".
description_text
Full guidebook description, verbatim.
description_condensed
Rewritten for a mobile screen without scrolling (54-74 words).
condensed_word_count
Word count of description_condensed.
reversed_reading_notes
Short phrase (8-15 words) translating upright meaning into blocked/shadow form.
description_anonymized
description_condensed with every identity tell removed (own name, own glyphs, rank
number, other-card comparisons) - for "is this an accurate description?" lessons.
Provenance: description_text/condensed trace to the guidebook. reversed_reading_notes and description_anonymized
are both authored, not guidebook-sourced - the guidebook has zero reversed material, and description_anonymized
was leak-checked against every card's own name/glyphs/suit.
Full data: Appendix G.
9. Symbol Images
File: data_symbol_images.csv | 30 rows | 1 row per symbol icon
Every Planet/Sign/Element had a dedicated icon already; the 4 Suits did not, so 4 were generated to match the
deck's gold line-art style. Full data: Appendix H.
10. Birth Card Lookup
File: data_birth_card_lookup.csv | 21 rows
Maps a birth date, reduced to 1-21 (see Section 17.3), to its Major Arcana card. The Fool (0) is intentionally absent -
the calculation never produces 0. Full data: Appendix I.
...
File: data_card_similarity.csv | 6,006 rows | every ordered pair of the 78 cards
Supports lesson formats that show a learner two or more cards and ask them to tell them apart. Each of the 3,003
unique pairs gets a computed similarity score from a one-time authored 6-axis thematic scoring pass
(connection/isolation, action/stillness, control/surrender, security/risk, inner/outer, ease/struggle), not from an
external source.
Column
Meaning
card_key_a / card_key_b
Both directions stored, so lookup works from either card.
mean_abs_diff
Raw closeness score (0-4 in practice). Lower = more similar.
similarity_score
1/2/3, cut at this deck's real 33rd/67th percentiles (not an even range split).
difficulty_tier
Hard/Medium/Easy spelled out explicitly, since the numbers alone are backwards: LOW
similarity_score (1) means confusable, which makes it a HARD lesson question.
Naming history: this column was originally called difficulty_score using the same 1/2/3 values, implying 1=easy/3=hard -
backwards from what it measures. A design thread hit this confusion directly. Renamed to similarity_score with
difficulty_tier added so it can't recur. Treat scores as a first-draft judgment call - the fix path is hand-editing specific rows,
not regenerating from axis scores (which aren't stored anywhere). Full data: not reproduced here (6,006 rows) - see the
CSV or database.
12. Teaching Order
File: data_teaching_order.csv | 78 rows | 1 row per card
A force-ranked sequence, easiest to hardest, from curriculum design - not derived from card_similarity. It reflects teaching
decisions (e.g. all four Aces early, court cards as a block) a similarity score alone wouldn't produce. Not meant to be
reconciled against difficulty_tier - "taught early" and "easy to distinguish" are different questions. Full data: Appendix J.
...
Zodiac Tarot App — Card Data Model Reference
13. Unit Metadata
File: data_unit_metadata.csv | 10 rows | 1 row per curriculum unit
Drives unit-select menus and unit intro screens directly (Zodiac_Tarot_Curriculum_Design_Spec.pdf, used by
ZodiacTarot_MainPath_Spec.pdf). This table pre-existed in the project as a design-thread output; it was not
previously in the database - added here for the first time.
Column
Meaning
unit_number
1-10. Primary key.
unit_name / unit_tagline / unit_intro_copy
Menu label, one-line hook, intro-screen copy.
section_count / standard_node_count /
throwback_node_count / total_node_count
Composition breakdown.
has_unit_recap / has_cumulative_review
Whether this unit ends with a recap and/or triggers review.
card_count / card_keys_covered
Which cards this unit teaches.
major_count / minor_count / dominant_suit
Composition summary.
est_completion_minutes_best_case /
_typical / _with_mistakes
Session-length estimates at 60s/90s/150s per node.
unlock_requirement
Gating rule for the unit-select screen.
icon_image_file / menu_order
Menu tile asset and display order.
Full data: Appendix K.
14. Curriculum Nodes
File: data_curriculum_nodes.csv | 577 rows | 1 row per playable lesson node
The full play sequence: 10 Units -> 78 Sections -> 577 Nodes, per Zodiac_Tarot_Curriculum_Design_Spec.pdf.
Column
Meaning
global_play_order
True sequential play order across the entire curriculum. Primary key.
unit_number / section_number_global /
section_number_in_unit
Position within the curriculum hierarchy.
card_key / card_name / arcana_type
The card this node teaches. Blank for unit_recap/cumulative_review rows, which
cover multiple cards.
is_symbol_intro
True if this section introduces a new symbol.
node_number
Order within its section (or unit-level group).
node_type
standard / throwback / unit_recap / cumulative_review.
node_id
Human-readable ID, e.g. U5-S40-N7.
format_code / format_name
Which lesson format to render (A1-A7, B, C) - see
Zodiac_Tarot_Lesson_Format_Bible.pdf.
anchor_variant
Which text/asset form anchors the round (e.g. keywords,
card_art_to_symbol_name).
node_phase
isolation / near_context / standing_skill / interleave_review / throwback_review /
unit_recap / cumulative_review.
cards_involved
Pipe-delimited card_key list. For A1/A2/A4/A5/A7, first key is the target, rest are
distractors.
cards_to_recall_count
Number of cards the learner must genuinely recall.
...
Meaning
distractor_tier
easy / easy_plus_one_known / n/a - confusability tier distractors were drawn
from.
notes
Implementation-facing explanation of this node's specific content rule.
This table pre-existed in the project as a design-thread output; it was not previously in the database - added here for the
first time, along with unit_metadata. Full 577-row data is not reproduced in this PDF (would run dozens of pages) - see the
CSV or database. A short sample is in Appendix L.
...
Page 9
Zodiac Tarot App — Card Data Model Reference
15. Account & Social Data (Meta Systems)
Per Spec_Meta_Hygiene_Systems.pdf: account creation, avatar selection, the bottom tab bar, the Friends tab,
friend progress markers, and the daily streak. This is the first data ask outside the tarot content layer - the earlier
reference to "users, progress, and social systems are a separate effort" in this document's own gap list (Section 19)
is what this section now begins to close.
15.1 avatar_sources — new table, real data
File: data_avatar_sources.csv | 78 rows | 1 row per card
Column
Meaning
avatar_id
1-78. Primary key.
card_key
Which card this avatar was cropped from. Not in the spec's own column list, added here for
provenance/traceability.
image_file
The pre-cropped avatar image filename, e.g. major_00_fool_avatar.png.
Correction to the spec's assumption: Spec_Meta_Hygiene_Systems.pdf describes the asset collection generically as
an "avatar_XXX naming pattern (avatar_001.png, avatar_002.png, ...)" and explicitly calls the exact contents "a
content/asset task, not a design decision." The real assets, generated by script_make_avatars.py and verified against all
78 files actually present in the project, are named {card_key}_avatar.png instead - one circular crop per tarot card, not a
generic sequential set. This table reflects the real filenames, not the spec's placeholder example. The picker itself (drawing
9 at random) works identically either way - only the naming assumption was wrong.
Full data: Appendix M.
15.2 users — new table, schema only
Not a CSV — schema-only, 0 rows, inside zodiac_tarot.db only
This is runtime account data, not authored content, so it ships with an empty table rather than a CSV of fake sample
rows.
Column
Meaning
user_id
...
username
Unique. Checked live at account creation; permanent - no rename flow exists anywhere in the
app.
avatar_id
Foreign key -> avatar_sources. Set once at creation; permanent - no avatar-change flow exists.
created_at
Timestamp. Also the Friends-list sort tiebreaker (earlier account ranks higher on equal
progress).
current_streak
Integer, defaults to 0. Incremented/reset per the spec's daily streak logic.
last_completed_date
Device-local date. Drives the once-per-day increment and missed-day reset.
Full data: none - this table has no rows to reproduce. See the database schema directly.
15.3 user_progress — referenced, not designed here
Deliberately not created in this database. Spec_Meta_Hygiene_Systems.pdf is explicit that this table is assumed to
exist (for computing path progress % and each friend's current-node marker) but belongs to a different, not-yet-written
curriculum/learner-progress spec. Guessing a schema for it here risks conflicting with whatever that spec eventually
defines. The one thing known about it: it needs a composite key of (user_id, node_id), where node_id refers to
curriculum_nodes.node_id.
...
Page 10
Zodiac Tarot App — Card Data Model Reference
Two computed values depend on this table once it exists, per the spec: path progress % (completed rows divided by
total nodes across all ten units, from unit_metadata) and friend clustering (which friends share a node - a runtime
grouping, not a stored relationship). Neither is a column anywhere - both are computed at read time.
...
Zodiac Tarot App — Card Data Model Reference
16. Challenge Data
Per Spec_Challenge_Tab.pdf: a head-to-head quiz a learner sends to one friend, built from the same lesson-format
vocabulary as the main Path, with hard-fail scoring and a live score-comparison for the receiver. Both players
answer an identical, frozen 10-question set, asynchronously - the receiver's question-by-question comparison
against the sender's already-recorded answers is the entire point of persisting the question set rather than
regenerating it.
16.1 challenges — one row per challenge, in either direction
Not a CSV — schema-only, 0 rows, inside zodiac_tarot.db only
Column
Meaning
challenge_id
...
challenger_user_id /
receiver_user_id
Both foreign keys -> users.
difficulty
Easy / Medium / Hard. Drives distractor confusability and grid/board size within whichever
format a question uses - not itself a format or content choice.
unit_cap
Integer, nullable. NULL = "All content"; when set, always >= 3, since both players must
have cleared the tab's own Unit 3 unlock gate first.
content_types
Pipe-delimited subset of Keywords / Description / Reading Notes / Shadow Meaning -
which content types this challenge draws questions from.
status
awaiting_receiver -> completed.
challenger_score /
receiver_score
Integers, nullable. receiver_score stays NULL until they finish playing.
outcome
challenger_win / receiver_win / tie, NULL while pending. A tie is a first-class value here, not
inferred from a null winner.
created_at / completed_at
Timestamps. Drive "recently challenged" sort order and history ordering. No expiry logic
reads these - an abandoned pending challenge is accepted as-is.
16.2 challenge_questions — the frozen 10-question set
Not a CSV — schema-only, 0 rows, inside zodiac_tarot.db only
Column
Meaning
challenge_id, question_order
Composite primary key. question_order is 1-10, generated once, never regenerated - this
table is read back question-by-question, not recomputed.
card_keys
Pipe-delimited; usually one card_key, more for multi-card formats.
format_code
A1 / A2 / A6 / B / C. Symbol formats (A3/A4/A5/A7) are never used here - none of
Challenge's four content types map to them.
content_type
Which of the four content types (below) this specific question serves.
difficulty_params
Grid size / scaffold variant / row count, per the difficulty table below. Frozen at generation
time.
distractor_set / correct_answer
Frozen at generation time - both players see the identical options.
challenger_answer /
challenger_correct
Written as the sender plays.
receiver_answer /
receiver_correct
Written as the receiver plays; read back live to drive the running score-comparison.
The four content types and where they come from
...
Keywords
card_keywords
A1 (keyword-list anchor), A2 (keyword-set
candidates), C (keyword rungs)
Description
card_descriptions.description_anonymized /
description_condensed
A1 (variant B), A2 (condensed-description
anchor), C (anonymized-text rung)
Reading Notes
card_talking_points
A1 (collated points), A2 (talking-points anchor), B
(True/False pool), C (reading-notes rung)
Shadow Meaning
card_descriptions.reversed_reading_notes
A6 only
Format A6 finally gets a home here. Zodiac_Tarot_Curriculum_Design_Spec.pdf explicitly excludes A6 (Shadow
Meaning Recognition) from the main Path, since it depends on a card's upright meaning already being taught. Challenge's
own Unit 3 unlock gate already guarantees that for every card in its pool, so A6 has no unmet prerequisite here and is
used freely - this is not a contradiction with Section 14's curriculum documentation, just a different context with a different
guarantee already satisfied.
Difficulty mapping (Section 16's three-tier selection, applied per format)
Format
Easy
Medium
Hard
A1 / A2
Grid of 2, Easy-tier distractors
Grid of 3, Medium-tier
Grid of 4, Hard-tier
A6
Scaffold A/B (upright description or
keyword anchor shown)
Scaffold C (single talking
point)
Scaffold D (no anchor at
all)
B
Easy-tier confusability on the donor
card
Medium-tier
Hard-tier
C
2 rows, easiest content-pairing rung
2 rows, hardest rung
3 rows, hardest rung
16.3 challenge_balances — head-to-head record per pair
Not a CSV — schema-only, 0 rows, inside zodiac_tarot.db only
Column
Meaning
user_id_a, user_id_b
The pair. Composite primary key.
wins, losses
Ties don't touch this table at all - per the spec, a tie moves neither player's balance.
Implementation choice, not a design decision: the spec explicitly flags that this table could instead be derived on read
from challenges, rather than stored and updated. Built here as a stored table since that's the schema the spec's own
data-requirements section lists, but treat it as changeable.
16.4 users.has_unseen_challenge_activity — the tab bar badge
Added as a single boolean column on the existing users table (Section 15.2) rather than a separate notifications
table, matching the spec's own "boolean/derived" framing of this as the simplest workable option. Drives the
Challenge tab's badge dot.
16.5 Reused content, unchanged
card_similarity (distractor confusability per tier), card_keywords / card_descriptions / card_talking_points (the four
content types' actual text), and unit_metadata.card_keys_covered (unioned across units 1..N to build the eligible
card pool) are all read here exactly as documented elsewhere in this reference - nothing about Challenge modifies
the content layer.
...
Zodiac Tarot App — Card Data Model Reference
17. Rules of Reading
How the guidebook says to use the deck, plus this app's own reversed-card rule. Documentation, not a queryable
table.
17.1 Preparing to read
Optional ritual before a first reading: a dedicated surface, a candle, water and sage to purify the hands. Consecrating
a new deck once, under a full moon or through incense smoke.
17.2 A Card a Day
Draw one card each morning (or evening, as recap). Look at the imagery first, before reading its meaning - building
trust in instinct before reference material.
17.3 The Birth Card and the Card of the Year
1
Add every digit of the full birth date (day + month + year) together.
2
If the result is greater than 21, add its digits together again.
3
Repeat until the result is between 1 and 21.
4
Look that number up in Birth Card Lookup (Section 10) for the matching card.
Worked example (28/01/1979): 2+8+1+1+9+7+9 = 37 → 3+7 = 10 → The Magician.
Special-case override: if the reduction lands exactly on 10 or 21, reduce once more anyway (10→1+0→1 The Magician;
21→2+1→3 The Empress), treating both as "turning-point" cards better passed through than landed on. This is calculation
logic, not a row in Birth Card Lookup.
Card of the Year uses the same method on just that year's digits. 2022 example: 2+8+1+2+0+2+2 = 17 → The Star.
17.4 Meditation on the Elements
Choose an Element, gather every card tied to it, observe recurring feelings and imagery, then sit with one Major
Arcana card that embodies it strongly.
17.5 Discovering the Signs
Gather every card tied to one zodiac sign (Aries: The Emperor + Two/Three/Four of Wands) and look for how the
sign's character recurs across them.
17.6 The Three Spreads
Single Card (the daily draw, 16.2), plus two formal layouts:
Three-Card Spread:
Pos.
Label
Guiding question
...
Where do I find myself in this moment? What is my starting condition?
2
The obstacle
What blocks my energy? What can I work on to free myself?
3
The development
...
1
Earth
Where can I find nourishment in this situation? What are my starting resources?
2
Air
What ideas might inspire me? How can I communicate better?
3
Fire
What moves me? What is my passion and my creative engine?
4
Water
What are my emotions, and how can I best manage them?
5
Intuition
A message from the cards for the situation being analyzed.
17.7 Reading Reversed Cards: Blocked / Shadow Energy
Authored for this app - not sourced from the guidebook, which has zero reversed material. The rule: a reversed card
is the same upright quality, expressed as blocked, delayed, excessive, or turned inward rather than flowing outward. Every
reversed_reading_notes entry (Section 8) follows this rule and assumes the reader already knows the upright meaning.
...
Page 15
Zodiac Tarot App — Card Data Model Reference
18. How to Access the Database
zodiac_tarot.db holds every table above as a real relational SQLite database, plus card/symbol/avatar images
embedded as BLOBs. No prior database experience assumed.
18.1 Opening the file

DB Browser for SQLite (free, graphical) - sqlitebrowser.org, File → Open Database.

Command line - sqlite3 zodiac_tarot.db

Any programming language - Python's sqlite3, Node's better-sqlite3, Prisma/SQLAlchemy.
18.2 All tables in the database
Table
Rows
Notes
tarot_cards_base
...
avatar_sources
78
Section 15.1, + image BLOBs.
users
0
Section 15.2 + 16.4. Schema only, populated at runtime.
challenges
0
Section 16.1. Schema only, populated at runtime.
challenge_questions
0
Section 16.2. Schema only, populated at runtime.
challenge_balances
0
Section 16.3. Schema only, populated at runtime.
card_images
78
Not a CSV. Full card artwork embedded as BLOBs.
Every card_key/user_id/etc. foreign key is enforced by the database itself.
18.3 Example queries
Get a card's full record:
SELECT b.card_name, s.symbol, d.description_condensed, d.reversed_reading_notes
...
Get a curriculum node's full context:
...
Page 16
Zodiac Tarot App — Card Data Model Reference
SELECT cn.node_id, cn.card_name, cn.format_code, um.unit_name
FROM curriculum_nodes cn JOIN unit_metadata um ON um.unit_number = cn.unit_number
WHERE cn.global_play_order = 1;
18.4 Getting images back out as files
Images are embedded as BLOBs for a complete, portable handoff - a real app should serve them as static files or
via a CDN. "Script to extract the images from DB.py" (Python standard library only) pulls all images back out using
their original filenames:
python3 "Script to extract the images from DB.py"
...
Page 17
Zodiac Tarot App — Card Data Model Reference
19. Known gaps, inconsistencies, and judgment calls
Missing keywords in the source guidebook (resolved)
Queen of Cups and Ace of Swords have no guidebook keyword line; King of Cups' were missing from an earlier
upload but present in the guidebook. The first two were authored to fill the gap, flagged source = "proposed" in Card
Keywords.
Uranus's governed signs conflict in the source text
The guidebook says Uranus governs one sign (Aquarius) in one place, two (plus Capricorn) in another. This
document uses Aquarius only.
Minor Arcana glyphs: spot-checked, not exhaustively verified
5 of 56 images were visually inspected directly; the rest filled in from the guidebook's stated list.
Suit icons did not exist originally (resolved)
4 new icons were generated to match the deck's style. The Wands icon (a plain rounded rod) is the weakest visual
match - worth a redesign if professional art is commissioned.
Interpreted meaning is deliberately excluded from Tables 2-4
Those tables hold only what's physically visible on a card face, kept independently verifiable against the artwork.
Condensed and anonymized descriptions are hand-rewritten, not algorithmic
Both were rewritten by hand against explicit rules (an 80-word budget; a leak-check against every identity tell) - not
generated mechanically. Verbatim original text is preserved alongside both.
Reversed reading notes are authored, not guidebook-sourced
The guidebook contains zero reversed-card material. reversed_reading_notes follows one explicit rule (Section
16.7), applied consistently across all 78 cards.
card_similarity's score column was renamed after causing real confusion
Originally difficulty_score (1=hard, 3=easy - backwards from the name's implication). Renamed to similarity_score,
with difficulty_tier added to spell out Hard/Medium/Easy explicitly.
Teaching order and card similarity are independent inputs
teaching_order reflects curriculum design decisions unrelated to card_similarity's confusability scoring. The two can
...
Page 18
Zodiac Tarot App — Card Data Model Reference
unit_metadata and curriculum_nodes pre-existed this database, added here for the
first time
Both were created in a separate design thread as CSVs and used directly by two specs (Curriculum Design, Main
Path) before ever being added to zodiac_tarot.db. This document is the first time they've been formally documented
alongside the rest of the schema.
avatar_sources corrects a wrong assumption in its own source spec
Spec_Meta_Hygiene_Systems.pdf describes generic sequential avatar filenames as a placeholder example. The
real, verified assets are card-derived ({card_key}_avatar.png). See Section 15.1.
user_progress is a known, unfilled dependency, now leaned on by three specs
Spec_MainPath.pdf, Spec_Meta_Hygiene_Systems.pdf, and Spec_Challenge_Tab.pdf all assume this table exists
and is queryable by unit/node, but none of them designs it. Challenge alone reads it three ways (Section 3's unlock
gate, Section 6.2's unit-cap min(), and the friend-eligibility filter). Not created here - see Section 15.3. Whoever
eventually specs this table should treat every one of these as a read against it, not a reason to redesign it piecemeal
per spec.
Challenge's format A6 usage looks contradictory next to Section 14, but isn't
Zodiac_Tarot_Curriculum_Design_Spec.pdf excludes format A6 from the main curriculum entirely.
Spec_Challenge_Tab.pdf uses it freely, because Challenge's own Unit 3 gate already guarantees the prerequisite
(upright meaning taught) that made A6 unusable on the main Path. Both statements are correct in their own context
- flagged here so it doesn't read as a data-model inconsistency.
challenge_balances could be a derived view instead of a stored table
Spec_Challenge_Tab.pdf itself flags this as an open implementation choice, not a locked design decision. Built as a
stored table here to match the spec's own data-requirements listing, but win/loss aggregates could equally be
computed on read from the challenges table.
The database is a derived artifact and must be manually rebuilt when a CSV
changes
This has already caused drift more than once during this project's construction. Treat any CSV edit as incomplete
until the database is rebuilt to match.
Broader gamification and real social features remain out of scope
XP and true friend-request/accept flows are explicitly deferred in the Main Path and Meta Systems specs
respectively. The Challenge tab is being designed in a separate thread not covered here.
...
zodiac_tarot.db
SQLite database
All 16 tables - Sections 2-17
Script to extract the images from DB.py
Python script
Section 18.4
data_tarot_cards_base.csv
78 rows
Section 2 / Appendix A
data_major_arcana_symbols.csv
22 rows
Section 3 / Appendix B
data_minor_arcana_attributes.csv
56 rows
Section 4 / Appendix C
data_card_keywords.csv
307 rows
Section 5 / Appendix D
data_card_talking_points.csv
345 rows
Section 6 / Appendix E
data_symbol_significance.csv
137 rows
Section 7 / Appendix F
data_card_descriptions.csv
78 rows
Section 8 / Appendix G
data_symbol_images.csv
30 rows
Section 9 / Appendix H
data_birth_card_lookup.csv
21 rows
Section 10 / Appendix I
data_card_similarity.csv
6,006 rows
Section 11
data_teaching_order.csv
78 rows
Section 12 / Appendix J
data_unit_metadata.csv
10 rows
Section 13 / Appendix K
data_curriculum_nodes.csv
577 rows
Section 14
data_avatar_sources.csv
78 rows
Section 15.1 / Appendix M
Guidebook Instructions from card pack.pdf
source PDF
Original guidebook
...
0
The Fool
major_00_fool_MASTER.png
major_01_magician
major
1
The Magician
major_01_magician_MASTER.
png
major_02_high_priestess
major
2
The High Priestess
major_02_high_priestess_MA
STER.png
major_03_empress
major
3
The Empress
major_03_empress_MASTER.
png
major_04_emperor
major
4
The Emperor
major_04_emperor_MASTER.
png
major_05_hierophant
major
5
The Hierophant
major_05_hierophant_MASTE
R.png
major_06_lovers
major
6
The Lovers
major_06_lovers_MASTER.pn
g
major_07_chariot
major
7
The Chariot
major_07_chariot_MASTER.pn
g
major_08_strength
major
8
Strength
major_08_strength_MASTER.
png
major_09_hermit
major
9
The Hermit
major_09_hermit_MASTER.pn
g
...
major_10_wheel_of_fortune_M
ASTER.png
major_11_justice
major
11
Justice
major_11_justice_MASTER.pn
g
major_12_hanged_man
major
12
The Hanged Man
major_12_hanged_man_MAS
TER.png
major_13_death
major
13
Death
major_13_death_MASTER.pn
g
major_14_temperance
major
14
Temperance
major_14_temperance_MAST
ER.png
major_15_devil
major
15
The Devil
major_15_devil_MASTER.png
major_16_tower
major
16
The Tower
major_16_tower_MASTER.png
major_17_star
major
17
The Star
major_17_star_MASTER.png
major_18_moon
major
18
The Moon
major_18_moon_MASTER.pn
g
major_19_sun
major
19
The Sun
major_19_sun_MASTER.png
major_20_judgment
major
20
Judgment
major_20_judgment_MASTER.
png
major_21_world
major
21
The World
major_21_world_MASTER.png
minor_wands_ace
minor
Wands
Ace
Ace of Wands
minor_wands_ace_MASTER.p
ng
minor_wands_02
minor
Wands
...
Two
Two of Cups
minor_cups_02_MASTER.png
minor_cups_03
minor
Cups
Three
Three of Cups
minor_cups_03_MASTER.png
minor_cups_04
minor
Cups
Four
Four of Cups
minor_cups_04_MASTER.png
minor_cups_05
minor
Cups
Five
Five of Cups
minor_cups_05_MASTER.png
minor_cups_06
minor
Cups
Six
Six of Cups
minor_cups_06_MASTER.png
minor_cups_07
minor
Cups
Seven
Seven of Cups
minor_cups_07_MASTER.png
minor_cups_08
minor
Cups
Eight
Eight of Cups
minor_cups_08_MASTER.png
minor_cups_09
minor
Cups
Nine
Nine of Cups
minor_cups_09_MASTER.png
minor_cups_10
minor
Cups
Ten
Ten of Cups
minor_cups_10_MASTER.png
minor_cups_page
minor
Cups
Page
Page of Cups
minor_cups_page_MASTER.p
ng
minor_cups_knight
minor
Cups
...
Grouped by card. "(proposed)" = not from the guidebook.
...
Grouped by card: label, condensed upright, reversed/shadow, anonymized, then original.
...
Anonymized: This character stands at both the start and end of a journey, chaos becoming the engine of a new cycle. They step
to the edge of a precipice unafraid, trusting a path will appear beneath their feet. Tied to mystery and quick, unconventional
thinking, this energy resists convention and welcomes the unknown, treating every departure as an invitation rather than a risk.
...
Anonymized: This character sees beyond the visible, working with the resources already within reach. They know magic lives in
ordinary things, available only to those who know how to use it. A quick, communicative planetary energy lends sharp wit and the
ability to view a situation from several angles at once. Their staff recalls a symbol of healing, and the tools laid before them show
that everything needed is already at hand.
...
Anonymized: This character lives on the threshold between worlds, receiving rather than acting. Where another figure acts, she
absorbs, holding the stories of others through a quiet, dreamlike gestation. Her light is borrowed, like reflected light, borrowing its
glow from elsewhere, delicate and indirect. She embodies the feminine, gestation, and trust in inner vision, offering protection and
care from behind her still, watchful veil.
...
Anonymized: This character follows a private moment of reflection with creation itself. She is Mother Earth incarnate, giving life
to a child, a project, or an idea, and reminding us to nurture ourselves as generously as we nurture others. A planet of pleasure
and beauty brings bodily wellbeing and the glow that comes from within, echoed in the crown of stars and ripened wheat framing
her throne.
...
Anonymized: This character sits ready to act, giving shape to the creative chaos that came before. He sets clear boundaries and
claims his own space without hesitation. He shares a springtime urgency, the instinct to sprout and push forward before
overthinking, ruled by a spark of drive that is combative, reactive, and quick to abandon what no longer excites, in pursuit of pure
momentum.
...
Anonymized: After a period of stability, this character introduces the first real crisis, a deeper search for meaning. He bridges the
everyday and the sacred, teaching that a spiritual perspective can carry us through hard moments. He's paired with a sensory,
present-focused sign that fully inhabits the here and now, holding a flame within earthly form.
...
Anonymized: A communicative, inquisitive mind sets the tone here. The card usually shows two figures beneath a watching
angel, forming a triangle of body, mind, and spirit. It centers on a fork in the road: have you weighed every part of the decision?
True choice comes from listening to all three voices together, not from the heart alone.
...
Anonymized: A cardinal sign is an initiator especially in matters of intuition, relationships, and feeling, and that same initiating
drive appears in this card of momentum and ambition. Two horses pull it in different directions, symbols of the forces moving us,
and only by aligning intuition with reason can we actually reach the goal. An emotional current becomes the fuel for the whole
journey.
...
Anonymized: This card depicts a lion: courage, personal power, and a sense of belonging rather than dominance. This isn't
physical force, it's a gentler energy that comes from within and radiates outward. It means following your heart honestly, acting
with generosity, and letting creativity flow in harmony with others, letting that softness guide and calm our more aggressive
...
Anonymized: A grounded, detail-oriented planetary energy quietly organizes here. This character masters a gift for solitude,
treating being alone as generative rather than lonely. Their lantern lights the path one careful step at a time, mirroring a patient
attention to detail. As the first card in the deck to carry light, this path leads inward, toward a spiritual center.
...
Anonymized: This card marks a turning point in the journey, the moment personal choice starts shaping destiny. Fortune and
misfortune are simply transitions, each one closing a cycle and opening another. A planet of expansion, generosity, and guidance
brings its energy here, its very name meaning 'guru' in Hindi. Even abundance needs balance, like standing steady atop a turning
wheel.
...
Anonymized: After a prior turning point comes real responsibility. This card holds up a mirror, an honest, unflinching reflection of
what's true. Its rules aren't written in books, they come from an inner sense of care toward the world. A sign ruled by a planet of
beauty and harmony brings its aesthetic, harmony-seeking gaze here, so equanimity becomes inseparable from what's fair.
...
Anonymized: Before a certain outer planet's discovery, this card belonged simply to Water: flowing, hanging, embracing.
Suspended upside down, this character's calm expression gives away the truth: they aren't suffering, they're meditating, gaining a
new perspective through stillness. Crossed legs echo a yoga pose. A planet of dreams, depth, and sacrifice, and Water's
unstoppable flow, capture the soul of this quiet surrender.
...
Anonymized: A fixed water sign belongs to a month when decay feeds new growth, just as falling leaves fertilize the roots
beneath them. Sitting between life and death, this energy is magnetic and intense, aware that reaching what's essential means
releasing what no longer has life in it. This card carries that same message: not an ending sealed shut, but an organic process
that clears space for fresh blood and new beginnings.
...
Anonymized: A mutable fire sign lives by the idea that the journey matters more than the destination, curious and torn between
instinct and a wish to belong. This card embodies alchemy itself, blending different ingredients into something greater than their
sum. Through experience and study, both energies described here temper diverse experiences into deeper understanding,
broadening their view of the world along the way.
...
Anonymized: Neither wholly good nor evil, this card recalls a mythic half-goat figure, embodying earthy, instinctual force. A
cardinal Earth sign is practical and control-oriented, especially about material life. Ruled by a planet of limits, it understands the
hard work required to reach a goal. This card speaks to that same responsibility: the chains shown are loose, not locked, since
our freedom is always a choice we're free to make.
...
Anonymized: A planet of raw energy, a mythic figure of war, is unafraid to stir what's grown stagnant. It speaks up against
injustice rather than staying silent. That same activist spirit strikes here: lightning shatters the structure, but what matters is how it
falls. Rather than resisting the change, it rides the wave, using that fire to survive and see beyond the old, outgrown way of
thinking.
...
Anonymized: A figure pours water onto both earth and river, an act that feeds the ground while also serving something purely
ideal, hope, and shared intention. A fixed air sign carries a double nature: airy and lucid, yet emotionally tied to Water. Ruled by
an outer planet that spins on its side, this energy marches to its own rhythm, valuing collective good over individual will. Here, that
uniqueness becomes hope itself, an openness to life and nature.
...
Anonymized: This card asks us to look closely at our hidden, intimate side, where the senses sharpen and everything takes on a
deeper, more magical dimension. It resonates with a mutable water sign, the zodiac's final and most inwardly guided sign, where
daily life and spirit meet, echoed in two fish swimming in opposite directions. Ruled by a planet of mystery, this card invites us into
the unknown to face repeating patterns and grow.
...
Anonymized: A card and its ruling planet share a name, radiating warmth, life, and true self-expression. Often shown with a
child, symbolizing purity, creativity, and optimism, this card brings fortune and success drawn from authentic identity rather than
performance. As the pulsing heart of the whole system, it offers vital light, but with a caution: get too close and you can be
burned, so avoid centering everything on yourself.
...
Page 49
Zodiac Tarot App — Card Data Model Reference
Anonymized: An outer planet of the underworld governs death, resurrection, and deep transformation, fitting for this card,
second-to-last in its sequence yet arguably the true conclusion of a much longer journey. Here we find our real calling and true
voice, but only by first passing through endings, rebirths, and transformations that reach into our depths. Its guidance is to release
old patterns and anything that no longer belongs, making room for healing.
...
Anonymized: A journey that began much earlier completes here. A planet that sets boundaries and cuts away the unnecessary
governs this final card, often shown with a wreath symbolizing wholeness and a closed circle. From here, all that's left is to begin
again, staying open rather than settling into comfort. That same planetary energy closes this cycle cleanly, clearing the way for
...
Anonymized: This card is fire in its rawest form, holding the renewing energy shared by its three related signs. It marks a
passionate new adventure, the moment the world says yes and dares you to act. It's the very first step toward a new idea, fragile
like the first spark, needing protection and fuel so it can grow into something lasting rather than fizzling out.
...
Anonymized: Every card of this number divides energy into dialogue and choice, and here two forces bring pure, active intensity
to that decision. The pull is impulsive and reactive, urging quick movement. This card asks whether your action should serve your
own small world or reach toward the wider community. Before acting, it's worth stepping back to take in the whole situation
clearly.
...
Anonymized: A warm, confident planetary energy sits comfortably in a bold sign here, lending courage and willful clarity to this
card. After an initial spark and a difficult decision, the project finally comes into focus, vision replacing uncertainty. We stand at
the threshold of a wider adventure, catching a glimpse of what awaits if we're brave enough to keep moving forward into the
unknown.
...
Anonymized: Two figures celebrate in warm, welcoming light, echoing a nurturing and an assertive energy meeting here. One of
these energies is somewhat out of its natural element, not entirely at ease with the other's combative nature, yet that friction
produces a stability worth marking. When opposites meet and settle, the result is something to celebrate: gatherings, plans finally
taking shape, and shared warmth after effort.
...
Anonymized: As with most cards of this number, this card brings a conflict to work through. A structured, limiting energy meets a
regal, expressive fire, so self-expression can feel constrained here. This energy's individuality also stands opposite a more
community-focused one, adding to the tension. The invitation is to turn inward, rediscovering strength and security in yourself,
letting the friction itself become raw material for something newly creative.
...
Anonymized: After a prior struggle, this card holds full self-expression: optimism, joy, and expansion, as a generous, lucky
energy meets a warm one. The battle is won and we're heading home to a personal celebration, earned fairly rather than at
anyone else's expense. Still, the card cautions against resting too long on this win. Enjoy it fully, but remember that luck is only
ever a season, not a guarantee.
...
Anonymized: A driving, assertive energy meets courage and will, pushing us into action. This card says we're ready to meet
whatever comes with valor and readiness, standing firmly behind our own values regardless of the cost. It asks us to show
ourselves fully rather than hide, since our example, like a generous kind of regality, can inspire others even in difficulty. This isn't
the moment to back down.
...
Anonymized: A quick, communicative energy and a direct, straight-talking sign amplify each other here. This card signals an
important message about to arrive, something long stuck suddenly freeing itself, a solution appearing right on time. It's a reminder
that you're exactly where you need to be, at exactly the right moment.
...
Anonymized: An intuitive, reflective energy takes on a restless one here, giving dreams a dynamic edge. This card marks a
pause for regrouping before the next move toward a goal, intuition sharpened into precision, like an arrow aimed carefully.
There's resilience here too, the strength to return to the fight even while wounded. Home, in this case, might be somewhere
unconventional, wherever lets you recover.
...
Anonymized: A limiting energy meets a love of expansion, an unlikely pairing that can actually teach real efficiency, if that
expansive side is willing to listen. This card warns against overextending: measure your effort, respect your resources, and don't
try to carry everything at once. It's also a reminder about delegating, since recognizing and accepting our limits is what actually
lets a plan reach completion.
...
Anonymized: Fresh, curious, and unafraid, this character blends airy inspiration with fiery energy, holding onto an early spark as
if daring the unknown with a child's open heart. They turn fear into vitality and creative drive, embodying a new project, journey, or
surprise worth embracing rather than avoiding. Their spirit trusts the exploratory, lively nature of this whole suit. It's time to follow
what excites you.
...
Anonymized: Where a younger figure merely imagines, this character can't wait to chase the idea down. Pure fire, they take
risks eagerly, acting first and thinking later, without a steadier figure's awareness. Their enthusiasm can burn out quickly after a
strong start, since pacing isn't their strength. They may signal sudden change or the need to move fast without overthinking;
consequences can be sorted out afterward. Nothing about them is boring.
...
Anonymized: Radiant and fully herself, this character knows her own power and isn't shy about using it. Unlike another figure's
healing energy, hers is creative and magnetic, driven and ambitious toward success. Charismatic enough to be noticed the
moment she enters a room, she's often shown with a black cat, hinting at intuitive, even witchy abilities she openly embraces
rather than hides. She knows exactly what she wants.
...
Anonymized: The true leader of this suit, this character roots a creative, transformative fire energy in steady earth. Honest and
capable, he leads others toward shared goals with real charisma, acting more cautiously than a more impulsive figure but sharing
another figure's gift for putting personal power at the community's service. Near him, people feel stronger and more capable,
warmed by an energy as steady and intense as a well-tended fire.
...
Anonymized: This suit belongs to Water and emotion, tied to universal love, relationship, and spirituality; like water left still,
feelings need to keep flowing rather than stagnate. This card blends a pure, domestic stream, a mysterious depth of death and
rebirth, and a boundless, healing sea, drawn from its three related signs. As a first card of its kind, it marks a true beginning:
infatuation, wonder, and new emotion arriving all at once.
...
Anonymized: A loving planetary energy entering a nurturing sign expands our emotional life, especially around love and
connection, exactly what this card shows: two figures crossing vessels in a gesture of closeness and discovery. It's the early
stage of a relationship, drawn to someone or something without yet knowing it deeply. This doesn't have to mean romance
specifically; it applies just as well to a new project, place, or experience.
...
Anonymized: Where one energy discovers emotion in a single relationship, a communicative one here shares feeling across a
whole group. This is the card of friendship, shared intention, and collaborating on something genuinely captivating. It reflects the
ease of being around people who speak your language and understand you instantly. Its abundance comes specifically from
shared experience, the wealth that only community, not solitude, can generate.
...
Anonymized: A reflective, emotional planetary energy is at home in a nurturing sign here, deepening feeling and the need for
protection and inner retreat. That sign's pull toward memory, nostalgia, and unrealized daydreams can tip into missing what's
actually being offered right now, symbolized by a cup being handed over unnoticed. This card is a signal to check where
emotional stagnation might be causing you to overlook a present opportunity.
...
Anonymized: A fixed water sign controls emotion to reach the heart of things, meeting a warrior-like, resolving energy here. A
figure stares at three spilled, empty vessels, echoing an abundance shown elsewhere in this suit, while two full vessels stand
untouched behind them. The card lingers on loss the way that fixed water sign would, but the warrior energy is present too,
quietly pushing toward transforming pain into awareness.
...
Anonymized: With a bright, illuminating energy meeting a tendency to preserve what it values, this card holds innocence,
childhood, and everything worth protecting emotionally. It draws on feelings already known and trusted, encouraging us to
cherish what once felt good and let that memory inspire the present. This is less about dwelling in the past than using it as a
source of comfort and encouragement now.
...
Anonymized: A loving planetary energy is somewhat out of its natural element in a deep, intense sign, turning it greedy for
feeling, desire, and fantasy, wanting to taste everything the vessels of wealth and success in this scene offer. The real theme
here is illusion and choice: are we seeing clearly, or are we the ones creating the deception? Any decision here deserves careful
evaluation rather than instinct, though pure, heartfelt desire can still point toward the right path.
...
Anonymized: An expansive, emotional sign meets a disciplined energy around time and limits. A figure walks away from familiar
vessels toward unknown, wild territory, marking a moment when emotions have stagnated and the only real option is to leave.
This card calls for abandoning old patterns and closing a cycle, embracing the shape of its number as an infinity of ending folding
...
Anonymized: A deep, emotional sign meets an expansive, fulfilling energy, making this the card of desires realized. If you want
something, this card suggests it's already on its way. Sitting just before completion, it holds a place of full, joyful anticipation
rather than impatience, a sign that something genuinely good is close at hand and worth savoring in the meantime.
...
Anonymized: A usually roaring, active energy finds calm here, anchored by a peaceful, harmonious sign. This card reflects real
satisfaction with what's been achieved, full presence, and everything settled comfortably in its place. The feeling is magnetic and
grounding, elevating everyday life rather than chasing something bigger. It's simply a genuinely happy moment, worth noticing
and enjoying rather than rushing past.
...
Anonymized: Air and Water combine here, inspiration mixed with feeling. This character works to turn dreams into reality,
marking the beginning of an exciting connection, whether with a person, a project, or an experience. Pure imagination and
lightness rooted in genuine feeling define them. Tied to childhood and innocence, they carry the very first seed of an emerging
love, still unfolding.
...
Anonymized: Water's romanticism meets Fire's passion in an unstable but powerful mix. This character can accomplish real
things through love, often drawn to art, whether painting, photography, theater, or music. They offer genuine gentleness and
warmth, but their fire can flare and fade quickly, making them inspiring yet unpredictable, someone who lives fully in both the
emotional and the creative world.
...
Anonymized: This character is pure Water, the emotional element in its fullest form. She loves without condition, listens with real
sensitivity, and heals through empathy rather than advice. Pure of heart, she isn't swayed by others' judgments, because she
sees straight through to the soul beneath them, meeting people exactly where they are.
...
Anonymized: This character grounds feeling through Earth, giving him real stability with emotion. Like another figure known for
the same suit, his listening itself transforms and heals others. Deeply connected to the arts, imagination is his defining power, but
he has to stay alert, careful not to lose himself in daydreaming. His strength lies in staying centered and rooted even while
working through deep feeling.
...
Anonymized: Like every card of its kind, this one is Air in its rawest form: sudden illumination, when a situation becomes
completely clear. It's the ability to see things exactly as they are, cutting away anything unnecessary to reach what's essential.
This can mean a new way of seeing something, or using reason without being pulled under by emotion. A central symbol in this
scene separates truth from deceit, echoing this element's gift for honest communication.
...
Anonymized: Hidden information here needs to be found within rather than outside. This secrecy reflects a balanced sign's
careful sensitivity, pausing to think before acting in order to restore inner balance. It's a card about searching and weighing
options, the quiet needed to make a real decision. A reflective planetary energy adds a sense of change, one that stabilizes
through that sign's naturally harmonious energy.
...
Anonymized: After a careful decision made earlier, this card faces its sometimes painful consequences. A disciplined energy
meets an emotional, beauty-seeking sign, an unusual but powerful pairing, since being in real relationship with the world means
being willing to feel pain fully. Without that, we stay tied to old hurt. This card breaks something open precisely so integration and
healing can follow.
...
Anonymized: An expansive planetary energy, also linked to fairness, meets a balance-seeking sign here. After a prior difficulty,
this card allows real healing: rest, an expanded view of things, and a fresh sense of harmony. This is compromise earned through
crisis and awareness, and through rest and acceptance a new path opens under a luckier sky.
...
Anonymized: A fixed, nonconformist sign meets a loving, exchange-oriented energy here. Someone wins and someone loses in
this card, though which role matters is left open, is it defeat, or victory at too high a cost? It echoes hope found elsewhere in this
deck: even in loss, hope for something better shouldn't be abandoned. This energy challenges convention, whatever the
immediate outcome.
...
Anonymized: An intellectual, communicative sign pairs naturally with a quick, exchange-oriented energy, both oriented toward
speed and exchange of ideas. This card traces a journey from an unsettled place toward calmer shores, a movement only
possible through trust. As with a difficult card before it, defeat is just a transition; here, moving forward means shifting from an old,
rigid idea toward something genuinely new.
...
Anonymized: Linked to a reflective energy's connection with a receptive figure elsewhere in this deck, this card turns a sign's
communication inward, becoming gentle, hidden, and cunning rather than direct. Thoughts need time to gestate before being
revealed, requiring tact rather than immediate disclosure. Though sometimes read as deception, it's more interesting as a quiet,
magical card about the discipline of waiting for the right moment to act.
...
Page 60
Zodiac Tarot App — Card Data Model Reference
Anonymized: A figure stands stuck and blindfolded here, an odd image for an energy usually about expansion. But that energy is
somewhat out of its natural element in a quick-thinking sign, so that sign's curious thinking turns against itself instead of
expanding outward. This card warns that overthinking and over-communicating can become their own trap. The obstruction is
self-made, and so is the freedom to release it.
...
Anonymized: A figure wakes from what looks like a nightmare, sharp blades behind her, evoking anxiety and inner conflict. A
passionate, intense energy meets a rational, communicative sign here, mind and heart colliding directly. The card's real comfort is
a reminder that things are rarely as dire as they seem in the dark. Morning light dissolves the shadows, if we're willing to look
again objectively.
...
Anonymized: This card falls at the very end of a communicative sign's cycle, ending right at a seasonal turning point tied to a
bright, radiant energy's own festival. A figure lies pierced by several blades, an image that seems at odds with that sign's usual
brightness, until you see the message: a cycle has fully ended, and clarity is needed to celebrate the dawn that follows. Let go of
the old to welcome the light.
...
Anonymized: Pure Air, this character personifies communication itself: processing information quickly and delivering unexpected
news or an important message. Often pictured high on a mountain, close to the sky, they're a natural student and investigator,
drawn to research and mental inquiry. One foot stays on the ground though, a reminder that even a mind this active still needs to
...
Anonymized: Fire meets Air here, a combination that can be explosive or, handled well, can light a warm and useful fire. This
character turns ideas into action, so the real question this card raises is whether that combination is balanced: is the situation
constructive or destructive, strategic or purely driven by passion? Focused on their goal, they still need to stay aware of what's
...
Anonymized: Water and Air combine in this character, letting her shape ideas through empathy and compassion rather than cold
logic alone. Often linked to a symbol of transformation, she turns emotion into lucid, direct, and sincere thought. Her wisdom
comes from experience, letting her weigh a situation from multiple angles and choose what's truly real. Like the tool she carries,
she cuts, but she also knows what's worth preserving.
...
Anonymized: This character channels sharp intellect through the steadying influence of Earth. A natural strategist and
communicator, he finds unexpected solutions to problems that seem impossible to untangle. His advice is to think outside familiar
patterns and gain distance for a clearer view; from above, the whole situation is easier to read. When this card appears, it's worth
listening to someone lucid and trustworthy.
...
Anonymized: This suit belongs to Earth, representing stability, material goods, and everything that nourishes and takes real
form. This card opens the door to new possibility in work, finances, or personal satisfaction, tied to self-care and protecting your
own boundaries. Like a seed breaking through dark soil, the opportunity here needs to be actively seized rather than left waiting,
since a card like this one is only ever a starting point.
...
Anonymized: This card is about finding balance across the ordinary demands of daily life, commitments, relationships, finances,
and work all at once. A lucky, growth-oriented energy meets a disciplined, hardworking sign here, an interesting combination of
expansion and restraint. Paying attention to how these two pulls alternate through your days, rather than favoring one, is what
brings genuine satisfaction and wellbeing.
...
Anonymized: This card carries the energy of building something real, staying focused long enough to actually finish what's been
started. A timely, active energy pairs with an orderly, tenacious sign's determination, giving you the tools to keep developing an
idea planted earlier without wasting effort. It can also point to teamwork, a group pooling different skills toward one shared result.
...
Anonymized: This card carries two intertwined meanings. As a card of its particular number, it represents stability and
foundation, the ability to protect yourself and what you've built. But paired with an earthy sign's nature and a bright energy's
presence there, it also hints at possible rigidity, a structure held too tightly. The lesson is to build solid ground first, then stay open
...
Anonymized: A fast, connective energy slows down considerably inside an earthy, patient sign. As with most cards of this
number, this card marks a recalibration after a prior stability, here showing up as material loss, hunger, and depleted energy. Its
real message is that isolation isn't necessary: asking for help and showing vulnerability is allowed. Turning inward at this moment
can also reveal unexpected resources.
...
Anonymized: A reflective, intuitive energy meets a grounded, material sign here, producing a slower, less reactive response to
need than fierier signs would give. This card is fundamentally about generosity, the give and take of resources. It asks an honest
question: right now, are you the one giving too much until you're depleted, or the one who needs to finally accept what you're
owed?
...
Anonymized: A figure often appears here tending seeds already sown, simply waiting for them to bear fruit. A disciplined energy
brings hard work and reflection that only pays off after patience, while a grounded sign knows how to wait for the right moment
before enjoying what's been cultivated. This card asks whether you're truly willing to be patient for the reward, since now is the
...
Anonymized: A grounded sign shows up here in its fullest form, embodying daily practice, careful attention to detail, and skilled
handiwork. This card favors getting to work, focusing on craft, and repeating small actions until mastery follows naturally. It's the
card of the apprentice, someone who keeps the bigger picture in view while patiently refining their skill. This is a moment to
devote yourself fully to your craft.
...
Anonymized: A loving planetary energy, tied elsewhere in this deck to nurturing and abundance, meets a grounded sign's
careful, daily devotion here, blending harmony and beauty with patient cultivation. A figure rests in their own garden, finally
enjoying what they've long tended. This is a card of independence: savoring what you've achieved without needing anyone else's
support, a rare permission to simply enjoy the fruit of sustained effort.
...
Anonymized: A communicative energy is at home in a grounded sign here, letting its gift for communication flourish through that
sign's organized, rational nature. This card holds wealth and generosity as both inner and outer qualities, bringing security and
serenity to home, family, and work. It's closely tied to gratitude, the satisfaction that draws in further blessings, and to the
...
Page 65
Zodiac Tarot App — Card Data Model Reference
inherited joy passed down through generations of belonging.
...
Anonymized: Air's intellectual curiosity interweaves with Earth's slow, careful pace in this character, the apprentice figure above
all others. This figure studies something new with genuine dedication, moving at a measured speed that honors every detail along
the way. In their grounded, earthy form, they remind us that enjoying the process itself matters more than rushing toward any
particular outcome.
...
Anonymized: The most careful of the four figures at this rank, this character lets Fire meet Earth, producing a measured pace
attuned to the natural rhythm of things. Fully dedicated to their mission, they favor hard work and steady study over leisure,
sometimes narrowing their focus to just one direction. What defines them is genuine willpower and constancy, exactly what's
needed to accomplish something genuinely complex.
...
Anonymized: An embodiment of Mother Nature, this character blends Earth with Water into fertile ground, practical and
responsive to whatever's needed. She feeds, nourishes, and finds real solutions within everyday life, her creativity showing
wherever she can offer sustenance. She also practices real self-care, treating her body as sacred. Rooted and present, she
experiences the world fully through the senses, just like a nurturing figure found elsewhere in this deck.
...
Anonymized: The generous leader among these court figures, this character has reached real personal fulfillment through strong
leadership. His wealth flows from a natural ability to share rather than hoard. Rooted fully in Earth, his wellbeing reflects real work
and dedication rather than luck alone, giving him a stability that feels earned. He gives freely precisely because he already
possesses genuine abundance.
...
Planet
Mercury
Planet_mercury_MASTER.png
Planet
Moon
Planet_moon_MASTER.png
Planet
Venus
Planet_venus_MASTER.png
Planet
Jupiter
Planet_jupiter_MASTER.png
Planet
Mars
Planet_mars_MASTER.png
Planet
Sun
Planet_sun_MASTER.png
Planet
Saturn
Planet_saturn_MASTER.png
Planet
Uranus
Planet_uranus_MASTER.png
Planet
Pluto
Planet_pluto_MASTER.png
Planet
Neptune
Planet_neptune_MASTER.png
Zodiac Sign
Aries
Zodiac_aries_MASTER.png
Zodiac Sign
Taurus
Zodiac_taurus_MASTER.png
Zodiac Sign
Gemini
Zodiac_gemini_MASTER.png
Zodiac Sign
Cancer
Zodiac_cancer_MASTER.png
Zodiac Sign
Leo
Zodiac_leo_MASTER.png
Zodiac Sign
Virgo
Zodiac_virgo_MASTER.png
Zodiac Sign
Libra
Zodiac_libra_MASTER.png
Zodiac Sign
Scorpio
Zodiac_scorpio_MASTER.png
Zodiac Sign
Sagittarius
Zodiac_sagittarius_MASTER.png
Zodiac Sign
Capricorn
Zodiac_capricorn_MASTER.png
Zodiac Sign
Aquarius
Zodiac_aquarius_MASTER.png
Zodiac Sign
Pisces
Zodiac_pisces_MASTER.png
Element
Fire
Element_fire_MASTER.png
Element
Water
Element_water_MASTER.png
Element
Air
Element_air_MASTER.png
Element
Earth
Element_earth_MASTER.png
Suit
Wands
Suit_wands_MASTER.png
Suit
Cups
Suit_cups_MASTER.png
Suit
Swords
Suit_swords_MASTER.png
Suit
Pentacles
Suit_pentacles_MASTER.png
...
Page 71
Zodiac Tarot App — Card Data Model Reference
Appendix K: Unit Metadata — full data
Unit 1: Instant Classics — Cards that click before you even know what tarot means
8 sections, 57 nodes (56 standard + 0 throwback). Recap: True. Cumulative review: False.
Cards: 8 (5 major, 3 minor, dominant suit: Cups (1/3)). Est. time: 57-142 min. Unlock: None (first unit)
Unit 2: Completing the Court — The last of the obvious majors, then your first full royal family
8 sections, 59 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: False.
Cards: 8 (3 major, 5 minor, dominant suit: Cups (4/5)). Est. time: 59-148 min. Unlock: Unit 1 complete
Unit 3: The Royal Court — Meeting everyone else who sits on a throne
8 sections, 60 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: True.
Cards: 8 (0 major, 8 minor, dominant suit: Wands (4/8)). Est. time: 60-150 min. Unlock: Unit 2 complete
Unit 4: Turning Points — Cards about motion, change, and consequence
8 sections, 59 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: False.
Cards: 8 (4 major, 4 minor, dominant suit: Swords (4/4)). Est. time: 59-148 min. Unlock: Unit 3 complete
Unit 5: Big Swings — The deck's most dramatic, unmistakable images
8 sections, 59 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: False.
Cards: 8 (4 major, 4 minor, dominant suit: Swords (2/4)). Est. time: 59-148 min. Unlock: Unit 4 complete
Unit 6: The Cups Story — One suit, start to finish
8 sections, 60 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: True.
Cards: 8 (0 major, 8 minor, dominant suit: Cups (8/8)). Est. time: 60-150 min. Unlock: Unit 5 complete
Unit 7: Wands in Motion — Passion and momentum, plus one deeper mind
8 sections, 59 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: False.
Cards: 8 (1 major, 7 minor, dominant suit: Wands (7/7)). Est. time: 59-148 min. Unlock: Unit 6 complete
Unit 8: Grounding Down — Where energy meets the material world
8 sections, 59 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: False.
Cards: 8 (0 major, 8 minor, dominant suit: Pentacles (6/8)). Est. time: 59-148 min. Unlock: Unit 7 complete
Unit 9: Sharpening the Mind — The Pentacles suit wraps up as Swords gets serious
8 sections, 60 nodes (56 standard + 2 throwback). Recap: True. Cumulative review: True.
Cards: 8 (1 major, 7 minor, dominant suit: Swords (5/7)). Est. time: 60-150 min. Unlock: Unit 8 complete
Unit 10: The Deep End — The most abstract cards in the deck, saved for last
6 sections, 45 nodes (42 standard + 1 throwback). Recap: True. Cumulative review: True.
Cards: 6 (4 major, 2 minor, dominant suit: Swords (2/2)). Est. time: 45-112 min. Unlock: Unit 9 complete
...
Page 72
Zodiac Tarot App — Card Data Model Reference
Appendix L: Curriculum Nodes — sample (first 20 of 577)
Full 577-row data is not reproduced here — see the CSV or database.
node_id
unit
card
format
...
Page 73
Zodiac Tarot App — Card Data Model Reference
Appendix M: Avatar Sources — full data
avatar_id
card_key
image_file
1
major_00_fool
major_00_fool_avatar.png
2
major_01_magician
major_01_magician_avatar.png
3
major_02_high_priestess
major_02_high_priestess_avatar.png
4
major_03_empress
major_03_empress_avatar.png
5
major_04_emperor
major_04_emperor_avatar.png
6
major_05_hierophant
major_05_hierophant_avatar.png
7
major_06_lovers
major_06_lovers_avatar.png
8
major_07_chariot
major_07_chariot_avatar.png
9
major_08_strength
major_08_strength_avatar.png
10
major_09_hermit
major_09_hermit_avatar.png
11
major_10_wheel_of_fortune
major_10_wheel_of_fortune_avatar.png
12
major_11_justice
major_11_justice_avatar.png
13
major_12_hanged_man
major_12_hanged_man_avatar.png
14
major_13_death
major_13_death_avatar.png
15
major_14_temperance
major_14_temperance_avatar.png
16
major_15_devil
major_15_devil_avatar.png
17
major_16_tower
major_16_tower_avatar.png
18
major_17_star
major_17_star_avatar.png
19
major_18_moon
major_18_moon_avatar.png
20
major_19_sun
major_19_sun_avatar.png
21
major_20_judgment
major_20_judgment_avatar.png
22
major_21_world
major_21_world_avatar.png
23
minor_wands_ace
minor_wands_ace_avatar.png
24
minor_wands_02
minor_wands_02_avatar.png
25
minor_wands_03
minor_wands_03_avatar.png
26
minor_wands_04
minor_wands_04_avatar.png
27
minor_wands_05
minor_wands_05_avatar.png
28
minor_wands_06
minor_wands_06_avatar.png
29
minor_wands_07
minor_wands_07_avatar.png
30
minor_wands_08
minor_wands_08_avatar.png
31
minor_wands_09
minor_wands_09_avatar.png
32
minor_wands_10
minor_wands_10_avatar.png
33
minor_wands_page
minor_wands_page_avatar.png
34
minor_wands_knight
minor_wands_knight_avatar.png
35
minor_wands_queen
minor_wands_queen_avatar.png
36
minor_wands_king
minor_wands_king_avatar.png
37
minor_cups_ace
minor_cups_ace_avatar.png
38
minor_cups_02
minor_cups_02_avatar.png
39
minor_cups_03
minor_cups_03_avatar.png
...
40
minor_cups_04
minor_cups_04_avatar.png
41
minor_cups_05
minor_cups_05_avatar.png
42
minor_cups_06
minor_cups_06_avatar.png
43
minor_cups_07
minor_cups_07_avatar.png
44
minor_cups_08
minor_cups_08_avatar.png
45
minor_cups_09
minor_cups_09_avatar.png
46
minor_cups_10
minor_cups_10_avatar.png
47
minor_cups_page
minor_cups_page_avatar.png
48
minor_cups_knight
minor_cups_knight_avatar.png
49
minor_cups_queen
minor_cups_queen_avatar.png
50
minor_cups_king
minor_cups_king_avatar.png
51
minor_swords_ace
minor_swords_ace_avatar.png
52
minor_swords_02
minor_swords_02_avatar.png
53
minor_swords_03
minor_swords_03_avatar.png
54
minor_swords_04
minor_swords_04_avatar.png
55
minor_swords_05
minor_swords_05_avatar.png
56
minor_swords_06
minor_swords_06_avatar.png
57
minor_swords_07
minor_swords_07_avatar.png
58
minor_swords_08
minor_swords_08_avatar.png
59
minor_swords_09
minor_swords_09_avatar.png
60
minor_swords_10
minor_swords_10_avatar.png
61
minor_swords_page
minor_swords_page_avatar.png
62
minor_swords_knight
minor_swords_knight_avatar.png
63
minor_swords_queen
minor_swords_queen_avatar.png
64
minor_swords_king
minor_swords_king_avatar.png
65
minor_pentacles_ace
minor_pentacles_ace_avatar.png
66
minor_pentacles_02
minor_pentacles_02_avatar.png
67
minor_pentacles_03
minor_pentacles_03_avatar.png
68
minor_pentacles_04
minor_pentacles_04_avatar.png
69
minor_pentacles_05
minor_pentacles_05_avatar.png
70
minor_pentacles_06
minor_pentacles_06_avatar.png
71
minor_pentacles_07
minor_pentacles_07_avatar.png
72
minor_pentacles_08
minor_pentacles_08_avatar.png
73
minor_pentacles_09
minor_pentacles_09_avatar.png
74
minor_pentacles_10
minor_pentacles_10_avatar.png
75
minor_pentacles_page
minor_pentacles_page_avatar.png
76
minor_pentacles_knight
minor_pentacles_knight_avatar.png
77
minor_pentacles_queen
minor_pentacles_queen_avatar.png
78
minor_pentacles_king
minor_pentacles_king_avatar.png
...
Page 75
Zodiac Tarot App — Card Data Model Reference
Appendix N: Image Gallery
Every card, symbol icon, and avatar in the deck, embedded directly in this document.
Major Arcana (22)
 The Fool
[major_00_fool]
 The Magician
...
Page 85
Zodiac Tarot App — Card Data Model Reference
Avatar Sources — sample (first 20 of 78)
Circular crops generated from each card's own art via script_make_avatars.py.
major_00_fool
 major_01_ma
 gician
 major_02_hig
